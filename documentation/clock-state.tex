...
\node (CLOCK_MODE_CHECK_ALARM) [decision, left=of CLOCK_MODE_SHOW] {\texttt{clock\_mode\_check\_alarm}};
...
\node (CLOCK_MODE_WFI) [state, left=of CLOCK_MODE_CHECK_ALARM] {\texttt{clock\_mode\_wfi}};
...
\node (USER_ALARM_MODE) [state, above=of CLOCK_MODE_CHECK_ALARM] {\texttt{user\_alarm\_mode}};
...
\draw [arrow] (CLOCK_MODE_SHOW) -- (CLOCK_MODE_CHECK_ALARM);
\draw [arrow] (CLOCK_MODE_CHECK_ALARM) -- (CLOCK_MODE_WFI);
\draw [arrow] (CLOCK_MODE_INCREMENT) -- (CLOCK_MODE_SHOW);
\draw [slow_arrow] (CLOCK_MODE_WFI) -- (CLOCK_MODE_CHECK_INTERRUPTS);

\draw [arrow] (CLOCK_MODE_CHECK_ALARM) -- (USER_ALARM_MODE);
\draw [arrow] (USER_ALARM_MODE) -| (CLOCK_MODE_WFI);
