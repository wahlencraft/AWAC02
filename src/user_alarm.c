...
void user_alarm_tests() {
    ...
    if (user_alarm_get_len() != 0) {
        ...
        if (user_alarm_exists(4, 7, 15)) {
            log(ERROR, "Found unexpected alarm (THI 07:15)\n");
        }
        if (user_alarm_exists(6, 9, 0)) {
            log(ERROR, "Found unexpected alarm (SAT 09:00)\n");
        }
        user_alarm_t alarm;
        alarm.dotw = 0;
        alarm.hour = 7;
        ...
        user_alarm_add(&alarm);
        if (!user_alarm_exists(4, 7, 15)) {
            log(ERROR, "Didn't find expected alarm (THI 07:15)\n");
        }
        if (!user_alarm_exists(0, 7, 15)) {
            log(ERROR, "Didn't find expected alarm (THI 07:15)\n");
        }
        if (!user_alarm_exists(6, 9, 0)) {
            log(ERROR, "Didn't find expected alarm (SAT 09:00)\n");
        }
        if (user_alarm_delete(0)) {
            log(ERROR, "Failed to delete user alarm 0\n");
        }
        if (user_alarm_get_len() != 1) {
            log(ERROR, "Unexpected user alarm len\n");
        }
        if (user_alarm_exists(4, 7, 15)) {
            log(ERROR, "Found unexpected alarm (THI 07:15)\n");
        }
